...
WMemoryFileMapper::WMemoryFileMapper(QObject *parent)
	: QObject(parent)
{
...
bool WMemoryFileMapper::Close(void)
{
	bool done = false;
	do
	{
...
//
//	Return the logical size of the WMemoryFileMapper.
//
WMemoryFileMapper::FileSizeType WMemoryFileMapper::Size()
{
	FileSizeType s = 0;
	if(m_file) {
		s = m_file->size();
...
bool WMemoryFileMapper::CopyContent(WMemoryFileMapper::FileSizeType offset,
								   WMemoryFileMapper::FileSizeType length,
								   QByteArray& to)
{
