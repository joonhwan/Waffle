#include "GVGraph.h"
#include "gui/graphviz/GraphViz.h"
#include "graphviz/gvc.h"

namespace {

inline char* makeChar(const QString& str)
{
	return const_cast<char*>(str.toLocal8Bit().constData());
}

/// The agopen method for opening a graph
static inline Agraph_t* _agopen(QString name, int kind)
{
    return agopen(makeChar(name), kind);
}
    ...
    if(itSource!=_nodes.end() && itTarget!=_nodes.end()) {
        ...
        if(!_edges.contains(key)) {
            Agedge_t* edge = agedge((Agraph_t*)_graph,
									(Agnode_t*)itSource.value(),
									(Agnode_t*)itTarget.value());
			if (edge) {
...
GVEdgeList GVGraph::edges() const
{
        ...
        it!=_edges.end();++it)
    {
        ...
        if((curve!=0) && (curve->size%3 == 1))
        {
            ...
            if(curve->sflag) {
                object.path.moveTo(curve->sp.x*scale,
								   (top - curve->sp.y)*scale);
                object.path.lineTo(curve->list[0].x*scale,
            ...
            for(int i=1; i<curve->size; i+=3) {
                object.path.cubicTo(curve->list[i].x*scale,
									...
									curve->list[i+2].x*scale,
									(top - curve->list[i+2].y)*scale);
			}

            //If there is an ending point, draw a line to it
            if(curve->eflag) {
                object.path.lineTo(curve->ep.x*scale,
